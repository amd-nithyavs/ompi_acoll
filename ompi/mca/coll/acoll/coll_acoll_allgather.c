...
static inline int rd_allgather_sub(void *rbuf, struct ompi_datatype_t *rdtype,
                                   struct ompi_communicator_t *comm, int count, int send_blk_loc,
...
static inline int mca_coll_acoll_allgather_intra(const void *sbuf, int scount,
                                                 ...
                                                 mca_coll_base_module_t *module)
{
    ...
    int sendto, recvfrom;
    int num_data_blks, data_blk_size[2] = {0}, blk_ofst[2] = {0};
    int bcount;
    int last_subgrp_rcnt;
    ...
    coll_allgather_decision_fixed(size, dsize * (unsigned long) rcount, sg_size, &use_ring,
                                  &use_lin);
    ...
    if (!(rank & (sg_size - 1)) && !use_rd_base) {
        ...
        for (i = 0; i < (num_sgs - 1); i++) {
            ...
            int scnt = (send_peer == last_brank) ? last_subgrp_rcnt : bcount;
            int rcnt = (recv_peer == last_brank) ? last_subgrp_rcnt : bcount;
