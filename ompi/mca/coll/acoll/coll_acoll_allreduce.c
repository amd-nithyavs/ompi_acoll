...
#ifdef HAVE_XPMEM_H
static inline int mca_coll_acoll_reduce_xpmem_h(const void *sbuf, void *rbuf, int count,
                                                ...
                                                mca_coll_base_module_t *module)
{
    ...
    int chunk = count / l1_gp_size;
    int my_count_size = (l1_local_rank == (l1_gp_size - 1)) ? chunk + count % l1_gp_size : chunk;
...
static inline int mca_coll_acoll_allreduce_xpmem_f(const void *sbuf, void *rbuf, int count,
                                                   ...
                                                   mca_coll_base_module_t *module)
{
    ...
    int my_count_size = (rank == (size - 1)) ? (count / size) + count % size : count / size;
    if (rank == 0) {
        if (sbuf != MPI_IN_PLACE)
            memcpy(tmp_rbuf, sbuf, my_count_size * dsize);
    } else {
                            ...
                            int up)
{
    ...
    tmp = (int *) ((char *) data->allshmmmap_sbuf[group[0]] + offset
                   + 64 * rank); /* ToDo: 64 should be replace by cacheline size */
    tmp0 = __atomic_load_n((int *) ((char *) data->allshmmmap_sbuf[group[0]] + offset
                                    + 64 * group[0]),
                           __ATOMIC_RELAXED); /* ToDo: 64 should be replace by cacheline size */
    ...
    if (rank == group[0]) {
        for (int i = 1; i < gp_size; i++) {
            volatile int tmp1 = __atomic_load_n(
                (int *) ((char *) data->allshmmmap_sbuf[group[0]] + offset + 64 * group[i]),
                __ATOMIC_RELAXED); // ToDo: 64 should be replace by cacheline size
            while (tmp1 == val) {
        ...
        volatile int tmp1 = __atomic_load_n(
            (int *) ((char *) data->allshmmmap_sbuf[group[0]] + offset + 64 * group[0]),
            __ATOMIC_RELAXED); // ToDo: 64 should be replace by cacheline size
        while (tmp1 != val) {
            tmp1 = __atomic_load_n((int *) ((char *) data->allshmmmap_sbuf[group[0]] + offset
                                            + 64 * group[0]),
...
int mca_coll_acoll_allreduce_small_msgs_h(const void *sbuf, void *rbuf, int count,
                                          struct ompi_datatype_t *dtype, struct ompi_op_t *op,
                                          struct ompi_communicator_t *comm,
                                          mca_coll_base_module_t *module, int intra)
{
    ...
    if (rank == l1_gp[0]) {
        if (l2_gp_size > 1) {
            mca_coll_acoll_barrier(data, offset2, l2_gp, l2_gp_size, rank, 3);
        }
    }
    ...
    mca_coll_acoll_barrier(data, offset1, l1_gp, l1_gp_size, rank, 1);
    ...
    if (intra) {
        err = ompi_coll_base_bcast_intra_binomial(rbuf, count, dtype, 0,
                                                  acoll_module->subc[comm_id].numa_comm, module, 0);
    }
    return err;
}
