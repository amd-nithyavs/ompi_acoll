...
static inline int coll_acoll_reduce_topo(const void *sbuf, void *rbuf, int count,
                                         ...
                                         mca_coll_base_module_t *module)
{
    ...
    if (rank == root) {
        ...
        tmp_rbuf = (char *) coll_acoll_malloc(reserve_mem_rbuf_reduce, dsize);
        if (NULL == tmp_rbuf) {
    ...
    if (is_base && (sz > 1)) {
        if (rank != root) {
            ...
            if (ret != MPI_SUCCESS) {
                free(pml_buffer);
                if (NULL != tmp_rbuf) {
                    coll_acoll_free(reserve_mem_rbuf_reduce, tmp_rbuf);
                }
                return ret;
            }
        }
...
#ifdef HAVE_XPMEM_H
static inline int mca_coll_acoll_reduce_xpmem(const void *sbuf, void *rbuf, int count,
                                              struct ompi_datatype_t *dtype, struct ompi_op_t *op,
                                              int root, struct ompi_communicator_t *comm,
                                              mca_coll_base_module_t *module)
{
    ...
    if (subc->xpmem_use_sr_buf == 0) {
        ...
        if (rank == root) {
            ...
            tmp_rbuf = (char *) coll_acoll_malloc(reserve_mem_rbuf_reduce, total_dsize);
            if (NULL == tmp_rbuf) {
                return OMPI_ERR_OUT_OF_RESOURCE;
            }
        }
